...
def _render_markdown(md: str) -> str:
    """Tiny, dependency-free Markdown → HTML for the AI insight card.

    Supports headings (``##``/``###``), ``**bold**``, ``- `` bullet lists and
    ``> `` blockquotes. All content is HTML-escaped first, so model output
    ...
    for raw in md.splitlines():
        ...
        elif line.startswith("&gt; "):  # '>' was escaped
...
class AlertVisualizer:
    ...
    def generate_all(
        ...
        # Keep figures so the dashboard can be rebuilt later with AI insights
        # (which depend on the report, generated after the charts).
        self.figures = [fig for _, fig in figures]
        dashboard = self.build_dashboard(self.figures)
    ...
    def build_dashboard(
        ...
        embedded as a plain ``<div>`` to keep the file small. When
        ``insights_md`` is provided (the optional LLM analyst briefing) it is
        rendered as a highlighted card at the top of the dashboard.
        """
        ...
        if insights_md:
            insight_card = (
                f'<section class="card insights">{_render_markdown(insights_md)}</section>'
            )
...
<style>
  ...
  .insights {{ padding:20px 24px; border-left:3px solid {_PALETTE['accent']}; }}
  .insights h2 {{ margin-top:0; color:{_PALETTE['accent']}; }}
  .insights blockquote {{ border-left:2px solid {_PALETTE['grid']}; margin:12px 0;
          padding:4px 14px; opacity:.8; }}
...
<footer>Generated by air-raid-analysis · plotly</footer>
        ...
        logger.info("Saved dashboard → %s%s", path, " (with AI insights)" if insights_md else "")
        return path
